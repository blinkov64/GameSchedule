...
<head>
	...
	<title>Game Schedule</title>
        ...
        <link rel="stylesheet" href="/public/css/adminPlace/update.css">
</head>
<body>
    <div class="container">
        <header>
        ...
        <div class="content">
            ...
            <div class="title">Update Place #<?php echo $placeId ?></div>
            <form method="post" action="/admin/place/update/<?php echo $placeId ?>">
                <label for="name">Name:</label>
                <div><input type="text" value="<?php echo ($place->name) ?  $place->name : $placeNameInput ?>" id="name" name="name"></div>
                <div class="error"><?php echo ($placeNameError) ? $placeNameError : ''?></div>
                <label for="address">Address:</label>
                <div><input type="text" value="<?php echo ($place->address) ?  $place->address : $placeAddressInput ?>" id="address" name="address"></div>
                <div class="error"><?php echo ($placeAddressError) ? $placeAddressError : ''?></div>
                <label for="active">Active:</label>
                <div>
                    <input type="radio" name="active" value="1" <?php echo ($place->active == '1') ? 'checked' : ''?><?php echo ($placeActiveInput == '1') ? 'checked' : ''?>>Yes
                    <input type="radio" name="active" value="0" <?php echo ($place->active == '0') ? 'checked' : ''?><?php echo ($placeActiveInput == '0') ? 'checked' : ''?>>No
                </div>
